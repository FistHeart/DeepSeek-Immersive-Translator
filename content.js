...
(function () {
  'use strict';
  let targetLang = 'zh-CN', hoverOn = false, articleOn = false, selectionOn = false, _obs = null;
  ...
  chrome.runtime.onMessage.addListener((m, s, r) => {
    ...
    if (m.action === 'getStatus') { r({ hoverOn, articleOn, selectionOn, cached: TransCache._mem.size }); }
  });
  ...
  function ensureObs() {
    ...
    _obs = new MutationObserver(Utils.debounce((mx) => {
      ...
      if (added.length) { DOMScanner.observeNew(added); ArticleTranslator.feedNewNodes(added); }
    }, 400));
  ...
  Storage.getPreferences().then(p => {
    ...
    if (hoverOn || articleOn) { ensureObs(); preload(); }
  });
})();

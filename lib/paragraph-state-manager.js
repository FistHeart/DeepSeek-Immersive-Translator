...
const ParaState = {
  ...
  // ── Block reference management (avoids nextElementSibling bugs) ──

  /** Store direct reference to translation block DOM element */
  _setBlock(el, block) {
    const entry = this._map.get(el);
    if (entry) entry._block = block;
  },

  /** Get stored translation block reference */
  _getBlock(el) {
    const entry = this._map.get(el);
    return entry?._block || null;
  },

  /** Clear stored block reference */
  _clearBlock(el) {
    const entry = this._map.get(el);
    if (entry) entry._block = null;
  },

...
const ArticleTranslator = {
  ...
  _initScan() {
    ...
    for (const el of nodes) {
      if (TransCoord.isPluginNode(el)) continue;
      if (TransCoord.isOwned(el)) continue;
  ...
  _safeInject(el, translation) {
    ...
    TransCoord.registerBlock(block, 'body-translation');
    ParaState.setTranslation(el, translation);
    ParaState._setBlock(el, block);
    console.log('[DTI] Injected block (owned): tag=' + el.tagName + ' parent=' + parentTag + ' id=' + id);
  },

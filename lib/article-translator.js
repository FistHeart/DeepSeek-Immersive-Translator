/**
 * Article Translator v5.5 — dual-path: _translateOne (manual) + _startLifecycle (auto batch).
 *
...
const ArticleTranslator = {
  ...
  //
  //  This is the DIRECT translation method for user-initiated actions.
  //  No batch queue, no 60ms window — translates immediately.
  //  Used by: RED click, error retry, watchdog recovery, expand-from-collapse.
  //

  /**
   * Translate a single paragraph immediately.
   * Independent of the auto-trigger batch system — always works on click.
   */
  async _translateOne(el) {
    ...
    console.log('[DTI] Direct translation started');
  ...
  // ── AUTO-TRIGGER TRANSLATION (RED indicator appears → auto batch) ──
  //
  //  This is the AUTOMATIC path. When RED indicators are inserted during
  //  detection, _startLifecycle(el, 'auto') collects them into a micro-batch
  //  and translates efficiently via batch API after a 60ms window.
  //
  //  Used by: _initScan, IntersectionObserver, feedNewNodes.
  //

  /**
   * Auto-trigger translation lifecycle — collects into batch, flushes after 60ms.
   * Used for automatic detection (NOT user clicks).
   */

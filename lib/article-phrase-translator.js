...
const ArticlePhraseTranslator = {
  ...
  _collapsePhrase(el) {
    ...
    if (entry?.block?.isConnected) {
      ...
      entry.block = null;
      // Remove after CSS transition completes (250ms fallback)
  ...
  _transition(el, state) {
    ...
    switch (state) {
      case 'detected':
        indicator.className = 'ds-phrase-indicator phrase-detected';
        indicator.style.cssText = ''; // Clear inline styles from success/loading
        indicator.title = '翻译已折叠，点击展开';
        indicator.style.cursor = 'pointer';
        break;

      case 'loading':
        indicator.className = 'ds-phrase-indicator phrase-loading';
        indicator.style.cssText = ''; // Clear inline styles
        indicator.title = '正在翻译...';
        indicator.style.cursor = '';
        break;

      case 'success':
        indicator.className = 'ds-phrase-indicator phrase-success';
        // Force visual update inline — bypass any CSS cascade issues
        ...
        console.log('[DTI] Phrase state → GREEN (success), entryId=', entry.id);
        break;

      case 'error':
        indicator.className = 'ds-phrase-indicator phrase-error';
        indicator.style.cssText = ''; // Clear inline styles

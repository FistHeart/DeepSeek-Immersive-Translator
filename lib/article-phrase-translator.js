/**
 * Article Phrase Translator v3 — independent phrase translation mode.
 *
...
const ArticlePhraseTranslator = {
  ...
  _injectTranslation(el, translation) {
    ...
    block.querySelector('.ds-phrase-retry').onclick = (e) => {
      ...
      this._refreshPhrase(el);  // Dedicated async-safe refresh method
    };
  ...
  /**
   * Refresh phrase translation — async-safe with timeout recovery.
   * Keeps old translation visible during reload; on failure, restores previous state.
   */
  ...
  /** Retry a failed phrase translation with full state guard */
  async retry(el) {
    const entry = this._phraseMap.get(el);
    if (!entry || entry.state === 'loading') return;
    this._removeTranslation(el);
    this._transition(el, 'detected'); // Reset to YELLOW before starting new cycle

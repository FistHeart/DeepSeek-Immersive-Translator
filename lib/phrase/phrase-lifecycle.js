...
const PhraseLifecycle = {
   ...
   *
   * Safe injection: for elements whose parent cannot contain arbitrary
   * siblings (UL, OL, TR, etc.), appends the box inside the element
   * instead of using afterend. Prevents browser DOM restructuring.
   */
  _renderBlock(entry) {
    ...
    // Safe injection: check parent for restricted content models
    ...
    if (parentTag === 'UL' || parentTag === 'OL' || parentTag === 'TR' ||
      ...
      // Restricted parent — append as child of element (valid HTML)
      block.classList.add('ds-ph-block--child');
      entry.element.appendChild(block);
      console.log('[DTI] Phrase block injected (child mode): entryId=' + entry.id + ' parent=' + parentTag);
    } else {
      // Normal parent — insert as sibling

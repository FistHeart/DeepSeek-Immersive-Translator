...
const HoverPopup = {
  ...
  _onMove: Utils.throttle(function(e) {
    ...
    if (TransCoord.isPluginNode(el)) return HoverPopup.hide();
  ...
  async _show(el, text, mx, my) {
    ...
    const v = TransValidator.validate(text, prefs.targetLang, 'hover');
    if (!v.valid) { this._renderPopup('error', prefs.targetLang); return; }
